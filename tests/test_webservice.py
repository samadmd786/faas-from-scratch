...
def test_fn_registration_invalid():
    # Missing payload
    resp = requests.post(base_url + "register_function", json={"name": "hello"})
...
def test_fn_registration():
    ...
    resp = requests.post(
        ...
        json={"name": "double", "payload": serialized_fn},
    )
...
def test_execute_fn():
    resp = requests.post(
        base_url + "register_function",
        json={"name": "hello", "payload": serialize(double)},
    )
    fn_info = resp.json()
    ...
    resp = requests.post(
        ...
        json={"function_id": fn_info["function_id"], "payload": serialize(((2,), {}))},
    )
...
def test_roundtrip():
    ...
    resp = requests.post(
        base_url + "execute_function",
        json={
            "function_id": fn_info["function_id"],
